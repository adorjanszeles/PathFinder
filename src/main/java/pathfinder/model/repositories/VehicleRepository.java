...
public interface VehicleRepository extends GraphRepository<Vehicle> {
	...
	@Query("MATCH (v:Vehicle) WHERE v.plateNumber CONTAINS {plateNumber} RETURN v")
	List<Vehicle> findByPlateNumber(@Param("plateNumber") String plateNumber);

	@Query("MATCH (v:Vehicle)-[r:OWNER]->(u:User) WHERE ID(v)={vehicleId} RETURN u")
	User findOwnerOfVehicle(@Param("vehicleId") Long vehicleId);

	@Query("MATCH (v:Vehicle)-[r:OWNER]->(u:User) WHERE ID(u)={userId} RETURN v")
